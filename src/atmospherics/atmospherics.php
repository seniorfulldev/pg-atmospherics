...
class Atmospherics
{
    ...
    protected function interpolateArray($array, $arrayIndex)
    {
        ...
        if ($arrayIndex <= $minIndex) {
            ...
            $integerPartOfArrayIndex = floor($arrayIndex);
            $decimalPartOfArrayIndex = $arrayIndex - $integerPartOfArrayIndex;
            $rangeSpread = $array[$integerPartOfArrayIndex + 1] - $array[$integerPartOfArrayIndex];
            $result = $array[$integerPartOfArrayIndex] + ($rangeSpread * $decimalPartOfArrayIndex);
        }
        return $result;
    }

    public function altitudeAdjustmentFactor($altitude)
    {
        return $this->interpolateArray($this->altitudeAdjustmentFactorTable, $altitude / 1000);
    }

    public function barometricPressureAdjustmentFactor($altitude, $barometricPressure)
    {
        // Compares the barometric pressure (inches Hg) at a given altitude (feet) to the standard barometric pressure at that altitude.
        $standardBarometricPressure = $this->interpolateArray($this->barometricPressureTable, $altitude / 1000);
        return ($barometricPressure - $standardBarometricPressure) / $standardBarometricPressure;
    }

    public function relativeHumidityAdjustmentFactor($temperature, $barometricPressure, $relativeHumidity)
    {
        // Compares the relative humidity (percentage) at a given altitude (feet) to the standard relative humidity at that altitude.
        $standardVaporPressureOfWater = $this->interpolateArray($this->vaporPressureOfWaterTable, $temperature / 2);
        return 0.995 * ($barometricPressure / ($barometricPressure - 0.3783 * $relativeHumidity * $standardVaporPressureOfWater));
    }

    public function speedOfSound($altitude)
    {
    ...
    public function speedOfSoundFactor($altitude){
        // The speed of sound factor compares the speed of sound (feet per second) at a given altitude (feet) to the standard speed of sound at sea level.
        return 1 - 0.00001126666 * $altitude - 0.00000000006753074 * pow($altitude, 2);
    }

    public function standardRelativeHumidity($altitude){
        // Compares the relative humidity (percentage) at a given altitude (feet) to the standard relative humidity at that altitude.
        $standardTemperature = $this->interpolateArray($this->temperatureTable, $altitude / 1000);
        $standardVaporPressureOfWater = $this->interpolateArray($this->vaporPressureOfWaterTable, $standardTemperature / 2);
        $standardBarometricPressure = $this->interpolateArray($this->barometricPressureTable, $altitude / 1000);
        return ($standardBarometricPressure - (0.995 * $standardBarometricPressure)) / (0.3783 * $standardVaporPressureOfWater);
    }

    public function temperatureAdjustmentFactor($altitude, $temperature){
    ...
    public function weightDensityOfAir($altitude){
        // Barometric pressure (inches Hg) at a given altitude (feet)
        return $this->weightDensityOfAirAtSeaLevel * exp(-0.0000302149 * $altitude);
    }
}
